...
class EDAToolsAgent(BaseAgent):
    ...
    missing data visualisations, correlation funnels, EDA reports, etc.
...
def make_eda_tools_agent(
    ...
    def exploratory_agent(state):
        """
        The main exploratory data analysis agent node.
        """
        ...
        # Extract all tool artifacts from messages
        eda_artifacts = {}
        for msg in internal_messages:
            if hasattr(msg, "artifact") and msg.artifact:
                if isinstance(msg.artifact, dict):
                    eda_artifacts.update(msg.artifact)
                else:
                    eda_artifacts[f"artifact_{len(eda_artifacts)}"] = msg.artifact
            elif isinstance(msg, dict) and "artifact" in msg and msg["artifact"]:
                if isinstance(msg["artifact"], dict):
                    eda_artifacts.update(msg["artifact"])
                else:
                    eda_artifacts[f"artifact_{len(eda_artifacts)}"] = msg["artifact"]
